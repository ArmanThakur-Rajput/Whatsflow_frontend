...
interface AdminStore {
  ...
  exportLeadsCSV: (dateFrom: string, dateTo: string) => Promise<string>;
...
export const useAdminStore = create<AdminStore>((set, get) => ({
  ...
  exportLeadsCSV: async (dateFrom: string, dateTo: string) => {
    const res = await axiosInstance.get(
      `/admin/leads/export?dateFrom=${dateFrom}&dateTo=${dateTo}`,
      { responseType: 'text' }
    );
